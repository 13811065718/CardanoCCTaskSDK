...
class CoinSelection {
  ...
  select(utxoSelection, outputAmount, limit) {
    try {
      ...
      if (e.message === 'INPUT_LIMIT_EXCEEDED') {
        ...
        // utxoSelection = this.descSelect(utxoSelection, outputAmount, limit);
        console.log("...error : descSelect: ", e);
        throw e;
      } else {
   ...
   * @return {UTxOSelection} - Successful random utxo selection.
   */
  descSelect(utxoSelection, outputAmount, limit) {
    ...
    do {
      ...
      if (selectedUtxosLen > limit) {
        console.log("\n\n...descSelect...selectedUtxosLen INPUTS_EXHAUSTED: ", selectedUtxosLen);
        throw new Error('INPUTS_EXHAUSTED');
      }
  ...
  randomImprove(inputs, outputs, limit) {
    ...
    //   console.log("randomImprove...coinsPerUtxoWord:", this.protocolParameters.coinsPerUtxoWord);
    //   let bnMinValue = CardanoWasm.min_ada_required(
    //     change, 
    //     false,
    //     CardanoWasm.BigNum.from_str(this.protocolParameters.coinsPerUtxoWord)
    //   );
    //   console.log("randomImprove...bnMinValue:", bnMinValue);
    //   let minAmount = CardanoWasm.Value.new(bnMinValue);
